...
namespace ZR.Admin.WebApi.Controllers
{
    ...
    public class TasksController : BaseController
    {
        ...
        public IActionResult Create([FromBody] TasksCreateDto parm)
        {
            ...
            tasksQz.ID = DateTime.Now.ToString("yyyyMMddHHmmssfff");
        ...
        public async Task<IActionResult> Stop([FromBody] SysTasksQz sysTasksQz)
        {
            if (string.IsNullOrEmpty(sysTasksQz.ID))
            {
                throw new CustomException("停止任务 Id 不能为空");
            }

            if (!_tasksQzService.Any(m => m.ID == sysTasksQz.ID))
            {
                throw new CustomException("任务不存在");
            }

            var tasksQz = _tasksQzService.GetFirst(m => m.ID == sysTasksQz.ID);

         //   string isStart = string.Empty;
            ApiResult taskResult  = null;
            if (tasksQz.IsStart == "0")
            {
                taskResult = await _schedulerServer.DeleteTaskScheduleAsync(tasksQz);
            }
            else
            {
                taskResult = await _schedulerServer.AddTaskScheduleAsync(tasksQz);
            }
            ...
            if (taskResult.Code == 200)
            {
                tasksQz.IsStart = sysTasksQz.IsStart;
                _tasksQzService.Update(tasksQz);
            }
