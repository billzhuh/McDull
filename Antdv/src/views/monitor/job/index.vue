  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      >
        ...
        <!-- <span slot="isStart" slot-scope="text, record">
          <a-popconfirm
            ok-text="是"
            cancel-text="否"
          ...
          >
            <span slot="title">确认<b>{{ record.isStart === '1' ? '开启' : '关闭' }}</b>{{ record.name }}的任务吗?</span>
            <a-switch checked-children="开" un-checked-children="关" :checked="record.isStart == 0" />
          </a-popconfirm>
        </span> -->
        <span slot="isStart" slot-scope="text, record">
          <a-popconfirm
          ...
          >
            <span slot="title"
              >确认<b>{{ record.isStart === '1' ? '启用' : '停用' }}</b
              >{{ record.name }}的任务吗?</span
            >
            <a-switch checked-children="开" un-checked-children="关" :checked="record.isStart == 0" />
          </a-popconfirm>
        </span>
...
<script>
import { listJob, delJob, runJob, changeJobStatus,exportTasks } from '@/api/monitor/job'
...
export default {
  ...
  methods: {
    ...
    confirmHandleStatus(row) {
      const text = row.isStart === '1' ? '启用' : '停用'
      row.isStart = row.isStart === '0' ? '1' : '0'
      changeJobStatus(row.id,row.isStart)
        .then(() => {
