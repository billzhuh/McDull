...
<script>
import { listType, delType, refreshCache,exportDict } from '@/api/system/dict/type'
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm({
        ...
        onOk() {
          return delType(dictIds).then((response) => {
            if (response.code == 0) {
              that.$message.error(response.msg, 3)
            } else {
              that.onSelectChange([], [])
              that.getList()
